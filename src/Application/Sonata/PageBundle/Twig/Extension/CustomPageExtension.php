        ...
        foreach($tobecreated as $key=>$value){
            ...
            $newblock->setParent($block);
             if($value['service'] == 'sonata.block.service.text')
                $newblock->setSettings(array("content" => "Insert your content heredddd"));
             elseif($value['service'] == 'sonata.media.block.feature_media')
                $newblock->setSettings(array("media" => false,"orientation" => "left","title" => null,"content" => null,"context" => "default","format" => "reference","mediaId" => null,'template' => $value['template']));
             elseif($value['service'] == 'sonata.media.block.gallery')
                $newblock->setSettings(array("gallery" => false,"title" => "etst","context" => false,"format" => false,"pauseTime" => 3000,"animSpeed" => 300,"startPaused" => 1,"directionNav" => 1,"progressBar" => 1,"galleryId" => null,'template' => $value['template']));
             elseif($value['service'] == 'sonata.media.block.gallery2')
               $newblock->setSettings(array("gallery" => false,"title" => "etst","context" => false,"format" => false,"pauseTime" => 3000,"animSpeed" => 300,"startPaused" => 1,"directionNav" => 1,"progressBar" => 1,"galleryId" => null));
             ...
             elseif($value['service'] == 'sonata.block.service.menu' || $value['service'] == 'sonata.block.service.footerlink'){
                 $newblock->setSettings(
                     array(
                         ...
                         'is_main' => isset($value['is_main'])?$value['is_main']:false
                     ));
                 if(isset($value['name'])){
                     $newblock->setName($value['name']);
                 }
